...
class BaseDataSetsMasked(Dataset):
...
def random_rot_flip_masked(image, label, mask):
...
def random_rotate_masked(image, label, mask):
...
class RandomGeneratorMasked(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() > 0.5:
            image, label, mask = random_rot_flip_masked(image, label, mask)
        elif random.random() > 0.5:
            image, label, mask = random_rotate_masked(image, label, mask)
        x, y = image.shape
        image = zoom(
